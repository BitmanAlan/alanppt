---
name: alanppt
description: Alan 的 PPT 生成 skill — 用单文件 HTML 生成横向翻页网页 PPT。两种网页风格：① "电子杂志 × 电子墨水"（衬线 + 流体 WebGL + 暖色）适合行业观察 / 人文 / 商业发布；② "瑞士国际主义 Swiss Style"（无衬线 + 22 锁定版式 + IKB/柠檬黄/柠檬绿/安全橙单锚点）适合数据 / 技术 / AI 产品汇报。第三条路径：可编辑 .pptx 麦肯锡咨询风（走 ~/dev/ppt/v2_image2_workflow 的 image-first 工作流，适合客户汇报与 PowerPoint 二次编辑）。第四条路径：多平台封面（公众号 21:9 头图 / 1:1 分享卡 / 小红书 3:4 / 视频号 16:9 / 短视频 9:16 竖封），复用同一套视觉系统但独立构图。触发词：做 PPT / 生成 PPT / 做幻灯片 / 做一份分享 / 杂志风 PPT / 瑞士风 PPT / Swiss Style / 网页 PPT / 麦肯锡 PPT / 可编辑 pptx / horizontal swipe deck / 做封面 / 头图 / 分享卡 / 小红书封面 / 视频号封面。
---

# Alan PPT Skill

> **来源声明**：本 skill 的网页 PPT 部分基于 [guizang-ppt-skill](https://github.com/op7418/guizang-ppt-skill)（作者：歸藏，MIT License）扩展而来。Alan 在此基础上叠加了"麦肯锡 pptx 第三条路径"、"多平台封面 D 路径"（针对中文社交平台 21:9 / 1:1 / 3:4 / 16:9 / 9:16 单图）、中文场景偏好与四路径决策表。这条信息只用于确认 Skill 来源，**不要写入生成的 PPT、HTML 页面、封面或配图**。
...
| **C · 麦肯锡可编辑 pptx** | .pptx | 客户咨询汇报 / 高管展示 / 需要在 PowerPoint 里二次编辑 | `~/dev/ppt/v2_image2_workflow/`（含 `image2_prompts.md` + `decompose_and_rebuild_prompts.md` + `make_mckinsey_v2.js`） |
...
C 路径的资源不在本 skill 目录里，在 `~/dev/ppt/v2_image2_workflow/`。工作流：

1. **生成 16:9 整页视觉**：用 GPT-Image-2（或 Nano Banana / Imagen 等）按 `image2_prompts.md` 里的 JSON Prompt 一次生成 3 张完整页面图
2. **拆分透明 PNG**：按 `decompose_and_rebuild_prompts.md` 的 Prompt 1，把每页拆成独立透明 PNG 视觉素材
3. **重建可编辑 pptx**：参考 `make_mckinsey_v2.js`（pptxgenjs 脚本），把 PNG 作为图层放回原位 + 加可编辑文本框
4. **产物示例**：`mckinsey_style_v2_3pages_editable.pptx`（可直接在 PowerPoint 里改文字）
...
## 这个 Skill 做什么（网页 PPT 部分 · 来自 guizang）
...
4. 走 A/B 路径时,第二问决定风格,然后:
   - 风格 A:读 `themes.md` 帮用户选一套主题色
   - 风格 B:读 `themes-swiss.md` 帮用户选一套主题色
5. **动手前 Read 对应模板的 `<style>` 块**——这是类名的唯一来源,缺类会导致整页样式崩
   - 风格 A → `assets/template.html`
   - 风格 B → `assets/template-swiss.html`
6. 读对应的 layouts 文件挑布局:
...
7. 如果风格 B 需要地点、路线、人物住所或城市关系地图,读 `swiss-map-component.md`
8. 如果在 Codex 中生成 deck 内嵌配图,读 `image-prompts.md` 挑图片类型、比例和基础提示词;如果是用户原始截图,先读 `screenshot-framing.md`,优先使用 `assets/screenshot-backgrounds/` 的内置背景资产
9. 细节调整时读 `components.md` 查组件(含 Motion 动效系统章节,主要服务风格 A;风格 B 的组件细节在 `layouts-swiss.md` 附录)
10. 生成后先运行 `node scripts/validate-swiss-deck.mjs path/to/index.html`,再读 `checklist.md` 自检
