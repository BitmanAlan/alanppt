...
用于 Codex 环境下为本 skill 生成 **PPT deck 内嵌素材**（A/B 路径用）。提示词只负责定基调,不要写成长篇说明。先判断图片落位和比例,再选择类型。

> 如果你要生成的是**独立单张封面**（公众号 21:9 头图 / 朋友圈 1:1 分享卡 / 小红书 3:4 / 视频号 16:9 / 短视频 9:16 竖封），请改用 `cover-specs.md` — 它的 Prompt 模板针对平台安全区做了不同处理,和 deck 内嵌素材规则**不一样**。
